...
include($cfg['plugins_dir'].'/sxgeo/classes/SxGeo.php');
...
if ($sx_bulkmode){
	...
	$SxCountry = new SxGeo($cfg['plugins_dir'].'/sxgeo/data/SxGeo.dat', SXGEO_BATCH);
	$SxCity = new SxGeo($cfg['plugins_dir'].'/sxgeo/data/SxGeoCity.dat', SXGEO_BATCH);
...
function sx_getCountryCode($ip=null){
	...
	if (!$SxCountry) $SxCountry = new SxGeo($cfg['plugins_dir'].'/sxgeo/data/SxGeo.dat');
...
function sx_getCountryId($ip=null){
	global $SxCountry,$sx_bulkmode, $sx_country, $sx_ip;
	if (is_null($ip)) $ip = $sx_ip;
	if (!$SxCountry) $SxCountry = new SxGeo($cfg['plugins_dir'].'/sxgeo/data/SxGeo.dat'); // Режим по умолчанию, файл бд SxGeo.dat
...
function sx_getCityInfo($ip=null){
	...
	if (is_null($ip)) $ip = $sx_ip;
	if (!$SxCity) $SxCity = new SxGeo($cfg['plugins_dir'].'/sxgeo/data/SxGeoCity.dat'); // Режим по умолчанию, файл бд SxGeo.dat
